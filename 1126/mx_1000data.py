...
dataset=DepthDataset('../../Data_1000')
...
def train(model, loss_fn, optimizer, num_epochs = 1, plot_every = 2):
    ...
    for epoch in range(num_epochs):
        ...
        if(epoch % plot_every == 0):
            with open('save1000/losses_ep'+ str(epoch)+ '.pickle', 'wb') as f:  # Python 3: open(..., 'wb')
                pickle.dump([train_losses,val_losses], f)
            torch.save(model.state_dict(), 'save1000/data1000_dict_ep'+str(epoch))
train(upsample,loss_log,adam_optim,num_epochs = 200)
